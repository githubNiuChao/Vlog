//
//  NCHWebViewController.m
//  PLMMPRJK
//
//  Created by NCH on 2017/4/9.
//  Copyright © 2017年 GoMePrjk. All rights reserved.
//

#import "NCHWebViewController.h"
@interface NCHWebViewController ()
...
@implementation NCHWebViewController
...
//        self.webView.scrollView.scrollIndicatorInsets = self.webView.scrollView.contentInset;
    ...
    [self.webView addObserverBlockForKeyPath:NCHKeyPath(self.webView, title) block:^(id  _Nonnull obj, id  _Nullable oldVal, id  _Nullable newVal) {
        
        if (!kObjectIsEmpty(newVal) && [newVal isKindOfClass:[NSString class]] && [weakself webViewController:weakself webViewIsNeedAutoTitle:weakself.webView]) {
    ...
    [self.webView.scrollView addObserverBlockForKeyPath:NCHKeyPath(self.webView.scrollView, contentSize) block:^(id  _Nonnull obj, id  _Nullable oldVal, id  _Nullable newVal) {
...
#pragma mark - NCHNavUIBaseViewControllerDataSource
...
- (UIView *)NCHNavigationBarLeftView:(NCHNavigationBar *)navigationBar
{
    ...
    self.backBtn.jk_origin = CGPointZero;
    
    self.closeBtn.jk_centerX = leftView.jk_width - self.closeBtn.jk_width;
...
#pragma mark - NCHWebViewControllerDataSource
// 默认需要, 是否需要进度条
- (BOOL)webViewController:(NCHWebViewController *)webViewController webViewIsNeedProgressIndicator:(WKWebView *)webView
{
    return YES;
}

// 默认需要自动改变标题
- (BOOL)webViewController:(NCHWebViewController *)webViewController webViewIsNeedAutoTitle:(WKWebView *)webView
{
    return YES;
}


#pragma mark - NCHWebViewControllerDelegate
// 导航条左边的返回按钮的点击
...
- (UIProgressView *)progressView
{
    if(_progressView == nil && [self.parentViewController isKindOfClass:[UINavigationController class]])
    {
        ...
        progressView.jk_width = kkSCREEN_WIDTH;
...
- (void)dealloc
{
    NSLog(@"NCHWebViewController -- dealloc");
