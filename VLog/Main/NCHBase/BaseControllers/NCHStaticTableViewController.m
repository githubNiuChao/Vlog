//
//  NCHBaseSettingViewController.m
//  NetLottery
//
...
#import "NCHStaticTableViewController.h"
...
@interface NCHStaticTableViewController ()

@end

@implementation NCHStaticTableViewController
...
{
    NCHWordItem *item = self.sections[indexPath.section].items[indexPath.row];
    
    
    NCHSettingCell *cell = [NCHSettingCell cellWithTableView:tableView andCellStyle:UITableViewCellStyleValue1];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // 带箭头的cell
    if([item isKindOfClass:[NCHWordArrowItem class]])
    {
        NCHWordArrowItem *arrowItem = (NCHWordArrowItem *)item;
...
- (NSMutableArray<NCHItemSection *> *)sections
{
    if(_sections == nil)
    {
...
- (NCHStaticTableViewController *(^)(NCHWordItem *))addItem {
    ...
    return  ^NCHStaticTableViewController *(NCHWordItem *item) {
