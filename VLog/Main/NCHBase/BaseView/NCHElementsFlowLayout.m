...
//  Copyright © 2017年 NCH. All rights reserved.
//
...
static const UIEdgeInsets nch_EdgeInsets_ = {20, 10, 10, 10};
...
/** 每一列的最后的高度 */
@property (assign, nonatomic) CGRect nch_LastAtrbsFrame;
...
- (void)prepareLayout
{
    ...
    //如果重新刷新就需要移除之前存储的高度
    //复赋值以顶部的高度, 并且根据列数
    self.nch_LastAtrbsFrame = CGRectMake(0, 0, self.collectionView.frame.size.width, 0);
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CGFloat rightLeftWidth = self.collectionView.frame.size.width - CGRectGetMaxX(self.nch_LastAtrbsFrame) - [self xMarginAtIndexPath:indexPath] - self.edgeInsets.right;
    ...
    if (rightLeftWidth >= w) {
        ...
        x = CGRectGetMaxX(self.nch_LastAtrbsFrame) + [self xMarginAtIndexPath:indexPath];
        y = self.nch_LastAtrbsFrame.origin.y;
    ...
    // 覆盖添加完后那一列;的最新高度
    self.nch_LastAtrbsFrame = atrbs.frame;
...
- (CGRect)maxHeightFrame
{
    __block CGRect maxHeightFrame = self.nch_LastAtrbsFrame;
    
    [self.nch_AtrbsArray enumerateObjectsUsingBlock:^(UICollectionViewLayoutAttributes * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
