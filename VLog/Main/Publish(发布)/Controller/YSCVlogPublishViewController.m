...
//#import "VLPublishPhotoRequest.h"
...
#import "VLVideoInfoModel.h"//VLVideoInfo_DescModel 正文标签model
...
UITextFieldDelegate,
VLPublishTagListViewControllerDelegate
>
...
@property (strong, nonatomic) NSArray<HXPhotoModel *> *photoArray;//要发布的图片数组
@property (strong, nonatomic) NSArray<HXPhotoModel *> *videoArray;//要发布的视频、视频为单选 所以只有一条数据


@property (strong, nonatomic) NSMutableArray<VLVideoInfo_DescModel *> * descTagModels;
...
- (void)viewDidLoad {
    ...
    self.descTagModels = [[NSMutableArray alloc] init];
...
- (void)publishTagListViewController:(VLPublishTagListViewController *)vc pusblishDescTagModel:(VLVideoInfo_DescModel *)descModel{
    ...
    [self.descTagModels addObject:descModel];
    [self.publishView.bodyText replaceRange:self.publishView.bodyText.selectedTextRange withText:descModel.name];
}
...
- (void)actionPublish:(UIButton *)button{
    ...
    [array enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([obj containsString:VLTextViewComposeParserBrandSubKey]) {
            ...
            NSString *subString = [obj substringToIndex:obj.length-VLTextViewComposeParserBrandSubKey.length];
            for (VLVideoInfo_DescModel* descModel in self.descTagModels) {
                if ([descModel.name isEqualToString:subString]) {
                    [muarr addObject:descModel];
                }
            }
            
        }else if ([obj containsString:VLTextViewComposeParserGoodsSubKey]){
            NSString *subString = [obj substringToIndex:obj.length-VLTextViewComposeParserGoodsSubKey.length];
            for (VLVideoInfo_DescModel* descModel in self.descTagModels) {
                if ([descModel.name isEqualToString:subString]) {
                    [muarr addObject:descModel];
                }
            ...
            VLVideoInfo_DescModel *model = [[VLVideoInfo_DescModel alloc] init];
            model.is_tag = NO;
            model.name = obj;
    ...
    if (kArrayIsEmpty(self.photoArray)) {
...
- (void)videoRequest{
    ...
    VLPublishRequest *requeset = [[VLPublishRequest alloc] initWithVideoUrl:[self.videoArray firstObject].videoURL];
    NSData *videoData = [NSData dataWithContentsOfURL:[self.videoArray firstObject].videoURL];
...
- (void)photoRequest{
    ...
    for (HXPhotoModel *model in self.photoArray) {
        
        //        VLPublishPhotoRequest *requset = [[VLPublishPhotoRequest alloc] initWithImage:model.previewPhoto];
        
        //        [requset nch_startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest * _Nonnull request, NCHBaseRequestResponse * _Nonnull baseResponse) {
        //
        //
        //        } failure:^(__kindof YTKBaseRequest * _Nonnull request, NCHBaseRequestResponse * _Nonnull baseResponse) {
        //
...
- (HXPhotoManager *)manager {
    if (!_manager) {
        ...
        _manager.configuration.maxVideoClippingTime = 60;
