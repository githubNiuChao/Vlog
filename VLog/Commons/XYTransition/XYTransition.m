...
- (void)pushAnimateTransition:(id<UIViewControllerContextTransitioning>)transitionContext
{
    ...
    //复制一个Cell用于显示动画效果
    __block UIImageView * snapShot =[[UIImageView alloc] initWithImage:[fromCellView jk_screenshot]];
    snapShot.backgroundColor = [UIColor clearColor];
    [containerView addSubview:snapShot];

    [snapShot setJk_origin:nowViewPoint];
    //计算缩放比例
    _animationScale = MAX([toVC targetTransitionView].jk_width, snapShot.jk_width) / MIN([toVC targetTransitionView].jk_width, snapShot.jk_width);
    ...
    [UIView animateWithDuration:self.animationDuration animations:^{
        ...
        [snapShot setJk_origin:toViewPoint];
...
//        [UIApplication sharedApplication].mainTabBar.tabBar.alpha = 0;
...
//    CGFloat offsetY = fromVC.navigationController.navigationBarHidden ? 0.0 : 64;
    //复制一份截图用于动画过程
    __block UIImageView * snapShot =[[UIImageView alloc] initWithImage:[toCellView jk_screenshot]];
    
    //计算缩放比例
    _animationScale = MAX(fromCellView.jk_width, snapShot.jk_width) / MIN(fromCellView.jk_width, snapShot.jk_width);
    
    [containerView addSubview:snapShot];
    snapShot.backgroundColor = [UIColor clearColor];
    //将目标View先放大到跟当前view一样大，然后在动画中缩小，实现动画pop效果
    snapShot.transform = CGAffineTransformMakeScale(_animationScale, _animationScale);
    [snapShot setJk_origin:CGPointMake(0, nowViewPoint.y)];
    //用于动画设置淡出缩小效果
...
//    kAppDelegate.mainTabBar.tabBar.alpha = 0;
    [UIView animateWithDuration:self.animationDuration animations:^{
        ...
        [snapShot setJk_origin:leftUperPoint]; //设置相对位置
